Shader "CustomProcessing/WaveJitterGlitch"
{
    HLSLINCLUDE
        ...
        {
            float strength = 0.5 + 0.5 *cos(_Time.y * _Frequency);
            
            // Prepare UV
            float uv_y = input.texcoord.y * _Resolution.y;
            ...
            float uv_x = input.texcoord.x + noise_wave_x;

            float rgbSplit_uv_x = (_RGBSplit * 50 + (20.0 * strength + 1.0)) * noise_wave_x / _Resolution.x;

            // Sample RGB Color-
            half4 colorG = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, float2(uv_x, input.texcoord.y));
            half4 colorRB = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, float2(uv_x + rgbSplit_uv_x, input.texcoord.y));
